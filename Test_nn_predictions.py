...
if __name__ == "__main__":
    ...
    model = load_model(os.path.join("NN_winner", "cnn_Open28.hdf5"))
    # Проверяем её архитектуру
    model.summary()

    # Загружаем картинку для теста предсказания её класса
    _path0 = os.path.join(os.path.join("NN_data", f"training_dataset_{timeframe_0}"), '0')
    images_class_0 = [f for f in os.listdir(_path0) if os.path.isfile(os.path.join(_path0, f))]  # картинки класса 0
    _path1 = os.path.join(os.path.join("NN_data", f"training_dataset_{timeframe_0}"), '1')
