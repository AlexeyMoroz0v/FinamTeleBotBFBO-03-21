...
async def save_candles_to_file(security_board, security_code, intraday, time_frame, skip_first_date, skip_last_date, four_price_doji, fp_provider):
    datapath = os.path.join(Config.FilePath)  # Путь сохранения файлов для Windows/Linux
    ...
    if file_exists and os.path.getsize(file_name) == 0:  # Если файл существует и пустой
        print(f'Файл {file_name} существует, но пустой. Удаляем его.')
        os.remove(file_name)
    if file_exists:  # Если файл существует
        print(f'Получение файла {file_name}')
        file_bars = await read_file_async(file_name)
        file_bars = pd.read_csv(StringIO(file_bars), sep='\t', parse_dates=['datetime'], date_format='%d.%m.%Y %H:%M', index_col='datetime')
        ...
        next_bar_open_utc = datetime(2023, 11, 1, tzinfo=timezone.utc)  # Берем дату, когда никакой тикер еще не торговался
...
if __name__ == '__main__':  # Точка входа при запуске этого скрипта
    ...
    portfolio = Config.portfolio[0]  # тикеры по которым скачиваем исторические данные
    ...
    # Потом написать код для норм активации данного файла а не то что щас

    i = 0
    while i < 2:
        if i == 0:
            asyncio.run(save_candles_to_file(security_board, portfolio, True, IntradayCandleTimeFrame.INTRADAYCANDLE_TIMEFRAME_M1, skip_first_date=skip_first_date, skip_last_date=skip_last_date, four_price_doji=four_price_doji, fp_provider=fp_provider))  # минутные бары
        else:
            asyncio.run(save_candles_to_file(security_board, portfolio, True, IntradayCandleTimeFrame.INTRADAYCANDLE_TIMEFRAME_M15, skip_first_date=skip_first_date, skip_last_date=skip_last_date, four_price_doji=four_price_doji, fp_provider=fp_provider))  # 15 минутные бары
        i += 1
